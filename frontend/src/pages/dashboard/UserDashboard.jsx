    ...
    <div className="dashboard-container">
      ...
      {confirmCancel && (
        <ConfirmModal
          ...
          onConfirm={async () => {
            ...
            try {
              const res = await fetch(
                `http://localhost:8000/api/bookings/${confirmCancel}/cancel`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${token}`,
              ...
              if (res.ok) {
                setConfirmationMessage(t("booking_cancelled"));
                setUpcomingBookings((prev) =>
                  prev.filter((b) => b.id !== confirmCancel)
                );
              } else {
                const data = await res.json();
          ...
          message={t("are_you_sure_cancel_booking")}
          type="confirm"
        />
      )}
    </div>
